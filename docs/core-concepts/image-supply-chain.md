...
- **The latest release cannot hide a safe intermediate one.** The monitor scans the registry head and, when different, the newest version that has already served the cooldown. It considers them newest-first and promotes the first one that passes. A fresh or vulnerable head therefore cannot strand the catalogue on a still older pin.
