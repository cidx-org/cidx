...
**Where that answer comes from for an entry still on a running repository.** From Grype alone, and the report says so (#312). `security-audit.yml` builds each image's ignore file from the entries accepted on that image's repository, so a live entry filters its own CVE out of its own scan results by construction — asking `matches` whether it has since been fixed can only ever return silence. Grype _moves_ a suppressed finding to `ignoredMatches` rather than dropping it, `fix.versions` included, and that record is the whole of the evidence; Trivy's `--ignorefile` deletes the finding and its report keeps no trace. So a fix only Trivy reported is not visible there, and an entry absent from **Fixed upstream** means nobody said — never that no fix exists. Closing that half is #311's, which needs an unfiltered scan alongside the filtered one.
...
**`live` is not the same as "nothing to look at".** An acceptance is never the right answer to a CVE that is fixed upstream, and a fix can land long after the entry was written — so a live entry with one is an entry that should not exist: a repin candidate, not a renewal. It stays live, it stays on file, and it is listed under **Fixed upstream** as well, from what Grype recorded as suppressed ([above](#judging-a-finding)). Nothing about the four states changes; the report just stops being silent about the one case where it matters most (#312).
