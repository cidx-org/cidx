...
**And what those four rebuilds carried was nothing.** The repin that followed was measured before it was made, old digest against new: every layer differed and the images had been rebuilt weeks apart, and the vulnerability sets were **identical at every severity** — nothing fixed, nothing introduced, the same base version underneath. That is the finding, not a footnote to it. The expected story was that continuous rebuilds against patched bases are the proposition hardened images are run for and the pin was holding the catalogue away from them; the measurement did not support it. What the pin actually costs, here, was the drift itself — running content the vendor had replaced, without knowing — and the answer to what that drift is worth turned out to be: on this occasion, nothing.

Which is the argument for the design rather than against it. A signal that fires often with an empty payload is precisely the one that must not be wired to automatic adoption: every one of those four promotions would have taken unreviewed content in exchange for no measured gain. Reporting costs a line in a summary and a scan a human can run in ten minutes; adopting spends a guarantee that cannot be bought back.
...
**Fail-closed on the evidence itself.** Trivy's report says nothing about whether `--show-suppressed` was passed — the field is simply omitted when a scan suppressed nothing — so a report produced without it is indistinguishable from one that hid nothing. Run against the audit's artifacts from the day before the flag landed, the new reading called four ansible entries obsolete; every one of them is still carried, and `prune -x` would have deleted the exceptions waiving them. So an absence only counts when the results can say what was taken out of them. The flag is per workflow run, so one sighting settles it for the directory; with none, every absence reads `unknown` and the report says which evidence is missing and where to get it.

**The audit states what it filtered, rather than the readers inferring it (#327).** The guard above is right about what a sighting proves, and blind about the one case that needs no sighting: **an ignore file with nothing in it hid nothing**, so every absence from the results it produced is an absence. No scan result can ever say that, because what an empty ignore file leaves behind is exactly the silence a dropped flag leaves.

That is not an edge case, it is the state the catalogue has been in since #303: all eighteen acceptances are past their date, every ignore file the audit writes is empty, nothing is ever recorded as suppressed — so `vuln prune` held every entry in `unknown` and `SECURITY-BASELINE.md` published every count as a floor, on the one ground that makes a count certain. Conservative for precisely the wrong reason.

So `cidx security vuln ignore --results <dir>` writes down what it built, next to the JSON the scan is about to write there: the image, the repository, how many entries went into the file and how many were left out as past their date. It measures nothing new — the same two numbers already went to stderr for the run log, where no reader of the artifacts could see them — and it invents no format: one small JSON file per image, filed under the naming convention the scan results already use, ignored by every reader that does not look for it.

Three states, then, and they must never read the same:

| The results say                      | An absence in them means     | Where it comes from                                         |
| ------------------------------------ | ---------------------------- | ----------------------------------------------------------- |
| something was recorded as suppressed | the CVE is gone              | Grype's `ignoredMatches`, Trivy's `--show-suppressed`       |
| the ignore file was declared empty   | the CVE is gone              | the declaration `vuln ignore --results` writes              |
| neither                              | nothing — `unknown`, a floor | the default, and what a directory produced before #327 gets |

The third is the posture #324 built and it is unchanged: a results directory that states nothing settles nothing, whether it predates this or was assembled by hand. The declaration is read as evidence, never as permission — a declared count above zero says something _was_ filtering and leaves the sighting to settle it, and the claim "no catalogue image carries this any more" needs every running repository to account for itself, not one of them.

`container-monitor.yml` builds no ignore file and writes no declaration, so its artifacts are read the third way. That is honest rather than ideal: nothing filtered its results either, and it has no step in a position to say so.
...
**Accepted is a subset of carried, and counting it took #311's evidence.** The audit generates each image's ignore file out of the entries accepted on that image's repository, so an accepted finding is deleted from that image's own results by construction — and a file counting only what the reports show published what the catalogue carries _minus the part it had already argued about_. On the same artifacts that reads 447 where it should read 465. Both scanners keep the suppressed half now (Grype in `ignoredMatches`, Trivy under `--show-suppressed`), so the accepted identifiers are read back out of it and counted where they belong. Only the accepted ones: Grype's own default rule drops indirect `linux-libc-dev` matches, 188 of them on the Rust image alone, and adding those would move this number by a scanner's defaults rather than by anything the catalogue ships. On results that cannot say what their ignore file removed the file says so and reads as a floor — the same three states as `unknown` above, applied to a count: an ignore file declared empty removed nothing, so there is nothing to add back and the number is the total it claims to be, while results that state nothing stay a floor (#327). Only the repositories actually carrying an acceptance are asked, since a repository with none had nothing subtracted from this count. `cidx security summary` prints the same caveat on the same number, because the page and the file are the same measurement and a reader finding it hedged in one place and not the other would be right to trust neither.
