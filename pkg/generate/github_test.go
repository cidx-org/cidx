...
// setVersion overrides the package-level build version for a test and
...
// TestBootstrapVersion locks in the version → install-suffix mapping:
// stable releases pin (with the `v` prefix go install requires), everything
// without an installable tag falls back to latest.
func TestBootstrapVersion(t *testing.T) {
	tests := []struct {
		version string
		want    string
	}{
		{"1.7.0", "v1.7.0"},  // release build (ldflags strip the v)
		{"v1.7.0", "v1.7.0"}, // already-prefixed version stays valid
		{"dev", "latest"},    // local build, no tag to pin
		{"", "latest"},       // defensive: empty version
		{"1.7.0-nightly.20260101.abc1234", "latest"}, // nightly: no installable tag
		{"garbage", "latest"},                        // defensive: unparseable version
	}

	for _, tt := range tests {
		t.Run(tt.version, func(t *testing.T) {
			setVersion(t, tt.version)
			if got := BootstrapVersion(); got != tt.want {
				t.Errorf("BootstrapVersion() with Version=%q = %q, want %q", tt.version, got, tt.want)
			}
		})
	}
}

// TestGitHub_BootstrapPinsGeneratingVersion is the regression test for
// cidx-org/cidx#163: a workflow generated by a released cidx must install
// that exact release in CI, not @latest. Otherwise the local binary (built
// from main) and the CI binary (last release) can carry different presets
// and the same commit passes locally but fails in CI.
func TestGitHub_BootstrapPinsGeneratingVersion(t *testing.T) {
	setVersion(t, "1.7.0")
	...
	if !strings.Contains(output, "go install github.com/cidx-org/cidx/cmd/cidx@v1.7.0") {
		t.Error("release build should pin bootstrap to its own version, got:\n" + output)
	}
	if strings.Contains(output, "@latest") {
		t.Error("release build must not fall back to @latest:\n" + output)
	}
}

// TestGitHub_DevBuildFallsBackToLatest verifies the dev-build fallback: a
// non-release build has no installable tag, so @latest (the historical
// behavior) is the best available bootstrap.
func TestGitHub_DevBuildFallsBackToLatest(t *testing.T) {
	setVersion(t, "dev")
...
// TestGitHubWithOptions_SelfBuildIgnoresVersion verifies the cidx-repo
// bootstrap is unaffected by the build version: it always builds from the
// checked-out source, which by definition matches the generating commit.
func TestGitHubWithOptions_SelfBuildIgnoresVersion(t *testing.T) {
	setVersion(t, "1.7.0")
	...
	if strings.Contains(output, "go install") {
		t.Error("self-build bootstrap must not contain `go install` even on release builds:\n" + output)
	}
	if !strings.Contains(output, "go build -o bin/cidx ./cmd/cidx") {
		t.Error("self-build bootstrap should use `go build`, got:\n" + output)
	}
