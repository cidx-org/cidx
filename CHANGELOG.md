...
- **pr**: say which step a failing check died on, so `cidx pr status` and `cidx pr watch` answer *why* and not only *which*. #347 taught both commands to name the failing check and to print its failing step and error excerpt, but `remote.PRChecks.FailedStep` was a field no provider had ever filled — the renderer was written against data that did not exist, which is the same lie as a config key nothing reads (#322, #352). A check run does not carry its steps; the Actions job behind it does, and no lookup is needed to reach that job because a GitHub Actions check run **is** its job — verified on run 30989450607, where check run 92251901265 `Test` is job 92251901265, naming `Run tests`. On that run `✗ Test` becomes `✗ Test — failed step: Run tests`. **The answer is cached per check**, which is the whole reason this is affordable: `pr watch` polls every few seconds, so a red run would otherwise re-request every failed job on every cycle — the rate-limit cost that had the work deferred out of #354. A completed job is a settled fact and re-running one mints a new check run with a new ID (verified on run 30898949737, whose second attempt renumbered every job), so one request per failed check is enough however long the watch lasts; a test drives five polls and fails on a second request. Entries are keyed by head commit and dropped whole when it moves, so a watch spanning a push does not describe the previous one. Only the checks the count calls failures are asked about, and only those posted by the repository's own workflows, another app's check run sharing nothing but the ID space with a job. **`ErrorLog` stays as it was, filled only from `output.summary`**: GitHub Actions summarises nothing, and the job log was weighed and rejected for it — ~400 KB per failed check to reach a tail that reads `Process completed with exit code 1`, which is also all the annotations endpoint carries and less than the step's own name already says. **GitLab fills neither, deliberately**: a job's `script:` is a flat command list the API reports no per-command result for, so there is no step to name — `failure_reason` is a category (`script_failure`), and printing it as a step would name nothing while reading as though it did — and the job trace is the same rejected download as GitHub's log (#355)
- **pr**: build the remote provider only when something needs it, so `cidx pr create --dry-run` previews anywhere. #276 stopped the preview *mutating* the repository but not *requiring* a remote: the command was wired through `withRepoAndProvider`, which resolves the origin URL and looks up a GitHub token before the action runs. Both are local lookups — no network, no mutation, which is why they survived that sweep — but they gated the preview all the same, so a clone whose `origin` is a filesystem path died on `unable to parse remote URL`, and a machine with no `GITHUB_TOKEN` and no `gh auth login` on `no GitHub token found`, in both cases before printing a line. That is the situation someone previewing a command is often in. `pr create` and `pr ready` now take the resolver `withRepoAndLazyProvider` hands out — the shape #227 gave the release commands — and the dry-run paths, which use no provider, never build one; the real runs resolve it at the step that talks to the remote and fail there as before. On a feature branch the "does this branch already have a PR?" lookup is now best-effort for the same reason: it saves a wasted run, it is not a precondition of describing one. The other commands wired eagerly were checked: `pr merge`, `pr edit`, the artifact and workflow commands all use the provider on every path, dry runs included, and `cpw` is the one remaining case of the shape (its provider is needed only to watch CI, after the commit and push are done) — left alone here because deferring it changes what an unusable remote *means* for a command that has already mutated the repository, which is its own decision (#350)
- **ci**: fail the build when the `go-test` command stops covering a package that holds tests. This repository overrides the catalogue's `go test -v ./pkg/... ./cmd/...` to `./...` because the godog suites live in the root package (#271) and, since #317, every CLI test lives in `internal/commands` — 19 files the default also skips, including the flag-placement census of #274, the alias guard of #335 and the workflow-summary tests of #339. Removing or narrowing that one override would silence the BDD suite and the whole CLI suite at once, and the Test job would stay green having run neither: the failure is invisible from the outside, which is what made it worth pinning rather than leaving to review. `TestTheTestPhaseRunsEveryPackageThatHasTests` resolves the command the way the runner does (preset plus `[containers.NAME]` overrides) and fails, naming them, on any package carrying a `*_test.go` the resolved patterns do not reach. It lives in `pkg/config` rather than beside the other project guards in the root package on purpose: the command it checks is what decides whether the root package is compiled at all, so a guard placed there would be skipped by the exact narrowing it exists to catch. The catalogue default is left as it is — widening it to `./...` is a behaviour change for every project using the preset and deserves its own decision (#344)
- **pr**: name the checks behind the count in `cidx pr status` and `cidx pr watch`. Both stopped at `✗ 4/5 passed`, naming neither the failing check nor the reason, so the step after a red run was always the web UI or `gh` — at the exact moment `cidx repo workflow rerun --failed` (#342) had made deciding between fixing, rerunning and ignoring possible from here. `remote.PRChecks` already carried every check's name, its failing step and an error excerpt; nothing read them past the counters. The status block and the closing report of a watch — animated and quiet alike — now list each failing check, its failing step when the provider names one, and a one-line error excerpt when it carries one. The set listed is exactly the set the count calls a failure, mirroring the provider's own rule (`skipped` and `neutral` pass, a completed run with any other conclusion does not, a commit status neither pending nor successful does not), because a block saying `4/5 passed` while listing two checks contradicts itself. Nothing new is fetched here: GitHub Actions leaves `output.summary` empty on its job check runs, so at this point the gain was the names alone; #355 below fills the failing step from the Actions jobs API (#347)
- **check**: print the verdict of `cidx check workflow` once, on stdout. The summary went out twice — through logrus to stderr and plain to stdout, in two different formats — so the conclusion of the command, the one line that has to be unambiguous, arrived in duplicate on screen while a script capturing stdout got a string that was not the one displayed. stdout now carries the human-readable verdict and the exit code carries it for anyone not reading; the same doubling on the "no workflows found" notice, the missing-workflow-file error and the validation errors (logged, then returned to urfave, which reports them again) is gone with it. `check drift` and `validate` were checked for the same defect and have none: neither routes a line through logrus, and `check drift`'s summary goes to stdout when there is no drift and to the error channel when there is — one channel each way. Scenarios in `features/cli/check_workflow_verdict.feature` run the real command and fail if any of it reappears (#345)
- **pr**: stop `cidx pr create --dry-run` pulling before it honours the dry run. On `main` the command ran `git pull` first, so a preview did the two things a preview must never do: it fast-forwarded the checked-out branch, and it could not be asked for at all without a reachable remote. The pull is now reported (`Would pull latest changes from main`) and performed only on the real run, the same lazy shape #227 gave the release commands. A test builds a repository whose origin is a path that does not exist and fails if anything reaches for it. The sweep of every other `--dry-run` found no second case of the same defect: `pr ready`, `release commit`, `release tag prepare|create|delete`, `run` and `cleanup` decide from local state alone, and the remote reads left in place — `pr merge` resolving the PR number it would merge, `release prepare` listing merged PRs, `release create` checking for an in-flight release branch, `artifact cleanup` listing what it would delete — change nothing and are what the preview reports, so removing them would only make the preview lie (#276)
- **run**: add `--stream`, which streams container output at the normal log level. A run on a CI runner is quiet by default (#87) — container output buffered, dropped on success — which suits a passing lint and not a test job, whose output is the evidence: the Test job printed `PHASE: TEST` then `go-test completed`, with every executed scenario invisible, and a green run of zero scenarios would have read identically. The only way out was `--verbose`, which also switches logrus to Debug and prints the raw JSON of every image pull, so the choice was "show nothing" or "show everything, noise included". `--stream` defeats the buffering and nothing else, and wins over `--quiet` when both are given. `ci.yml` drops the `--verbose` it carried as a workaround since #271 for `./bin/cidx run --stream test` (#273)
