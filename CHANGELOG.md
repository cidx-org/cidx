...
- **cli**: every user-facing hint names a command that still exists — the `cidx action …` tree has been hidden and deprecated since the CLI was reorganized around the product core (2026-04), yet 24 hints in the release, tag, artifact and PR code kept sending users to it, joined by 73 references in the guides, the CLI reference and the example configs. The rewrite is not a prefix strip: `cidx action tag …` lives at `cidx release tag …` and `cidx action artifact …` at `cidx repo artifact …`, so dropping `action` would have produced commands that resolve to nothing, while `pr` and `cpw` do have top-level aliases and keep the short form. A test now fails if `cidx action` reappears in any non-test Go source (#174)
- **cli**: `cidx pr merge` states in its help that it is non-interactive — it merges as soon as the required checks are green but exposes no `--yes`/`--force`, so the flag list and the actual behaviour read as a contradiction. A confirmation prompt would break the unattended use the command exists for, so the contract is documented instead: the help says no confirmation is asked and points at `--dry-run` to preview, consistent with how the other destructive commands are gated (`repo branch cleanup` stays a dry-run until `-x`, `release tag delete` needs `--force` for protected tags) (#217)
- **drift**: `check drift` compares a workflow against the pipeline that workflow implements instead of the union of every pipeline — any project splitting release phases into their own workflow was permanently "drifted", including this repo, where `docker` and `release` come from `[pipelines.release]` and correctly have no job in `ci.yml`. The workflow's basename selects the pipeline (`ci.yml` ↔ `[pipelines.ci]`, `release.yml` ↔ `[pipelines.release]`, the mapping `check workflow` already used); a name matching no pipeline still compares against all of them, which is what the single `cidx.yml` written by `generate github` needs. The scope is printed above the table, and a phase of the compared pipeline missing from the workflow — or a job the pipeline never declares — is still reported (#178)
- **release**: `release create` now watches the run its own tag push triggered instead of probing the CI workflow candidates (`cidx.yml`, `ci.yml`) on a "branch" named after the tag — a tag push starts `release.yml`, so the last step of every release degraded to `Could not get workflow for tag vX.Y.Z` and sent the user to the releases page, which is why v2.1.1 → v2.1.4 had to be watched by run id by hand. Runs are resolved by the ref that triggered them (GitHub records it in `head_branch`, which for a tag push is the tag name), so the release run is found whatever its file is called while the CI and nightly runs of the very same commit are left out; the lookup is retried for up to 90s because the run is not registered the instant the push returns. `cidx workflow watch --tag <tag>` exposes the same selector for the manual case (#223)
- **config**: `required_version` is compared numerically and treated as a floor instead of an exact string — a config pinned to `v2.1.4`, the form every tag and doc example uses, never matched a release binary reporting `2.1.4`, and an exact pin would break every config on each cidx release. Both spellings are now equivalent, any newer cidx satisfies the requirement, an older one is refused with an upgrade hint, and a malformed value is named (#212)
- **release**: one conventional-commit parser for the whole release flow, so `feat!: …` is finally recognized as breaking — the regex behind `release prepare` stopped at the colon, so a breaking commit was typed `other` and produced a patch bump while `tag preview` proposed the major one. `prepare`, `tag preview` and the release TUI now classify commits through `actions.ParseCommit` and can no longer disagree on a version (#175)
...
- **pipeline**: `run <name>` accepts a container declared only through `[containers.NAME]` with an `image` field — target resolution knew pipelines, phases and built-in presets, so a custom container was answered `unknown target` even though the very same container ran fine once a phase included it. Custom declarations now take the `RunTool` path a phase already uses, and a genuinely unknown name is refused with a message naming every place a target can be declared (#216)
- **generate**: generated GitHub workflows carry `permissions: contents: read` at the workflow level and `persist-credentials: false` on every `actions/checkout` — the jobs only check out the repo and run containers, so they took the repository's default (often write-permissive) `GITHUB_TOKEN` scope for nothing and left that token in the git config while third-party images ran against the workspace. Both hardenings had to be hand-patched back after each regeneration (#207)
