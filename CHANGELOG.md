...
- **security**: key vulnerability exceptions by **repository and CVE** instead of by `repo:tag`, and triage findings before they reach a human. An exception records a judgement about a CVE in an image, in our usage; none of that changes when a tag moves, so `known-vulnerabilities.toml` now carries `repository` with the reference it was first seen on kept as `first_seen` context. The 33 entries on file were migrated by `cidx security vuln prune -x` rather than converted: a whole `repo:tag` matches no repository, so each fell through to the CVE test and was re-filed onto whichever repository the audit's findings show carries it today — 13 to `rust` (the kernel CVEs filed against `golangci-lint:v2.6.2`, which stopped shipping `linux-libc-dev` when it moved to Alpine), 20 to `ghcr.io/ansible/community-ansible-dev-tools`, collapsing onto 6 keys. 33 entries became 19, no justification lost. `vuln prune -x` now re-files carry-over entries as well as removing obsolete ones, both mechanical consequences of the CVE criterion; `unknown` is still never touched. Two triage rules land with it: a CVE with a fix upstream is image freshness, never an exception, so `cidx security vuln add` **refuses** to record one and names the fix (no `--force` — the correct action is a repin), and two classes are exempt by construction — the Go standard library compiled into a CLI binary (package `stdlib` in a `gobinary`/`go-module`, nothing else in the same binary) and the kernel headers package (`linux-libc-dev`, `linux-headers` exactly, not `util-linux`). A finding the two scanners disagree about stays in the queue. `SECURITY-BASELINE.md` now states what is carried and what is accepted separately, as the policy requires: **594 HIGH/CRITICAL carried** across the catalogue — 40 Go stdlib, 130 kernel headers, 208 fixed upstream, **216 needing triage** — against 32 accepted, where it used to publish "0 accepted findings" and nothing else. KEV and EPSS travel through from Grype (Trivy reports neither) and are printed for a human; neither gates anything. `known-vulnerabilities.toml` is written by hand rather than by the TOML encoder, so a removal reads as a removal instead of a 538-insertion reindent (#238, #289)
...
- **presets**: run `cargo-audit` off `buildpack-deps:trixie-curl` instead of the Rust toolchain, **179 HIGH/CRITICAL → 41**, and the catalogue 594 → 456 (150 needing triage, down from 216). #286 had to leave this one preset on the full `rust:1.97.0` because `-slim` ships no HTTP client and the preset downloads the RustSec release binary — but the variant was never the question: auditing a `Cargo.lock` reads a text file against an advisory list, and one preset was holding the catalogue's worst image, 179 findings against 415 for the other twenty put together, to run a binary it fetches anyway. `buildpack-deps:trixie-curl` is the Docker Official base `rust:` is itself built on, minus the toolchain, and carries exactly what the command needs: `sh`, `curl`, `tar`, CA certificates. Deliberately **not** alpine, which measures 0/0 at 4 MB: RustSec publishes a musl asset for `x86_64` only — `cargo-audit-aarch64-unknown-linux-musl` is a 404 — so a musl base would buy the last 41 findings by breaking every aarch64 user, #195 with the arrow reversed. glibc keeps `$(uname -m)-unknown-linux-gnu` resolving on both, verified under `linux/arm64` as well as `linux/amd64` detecting RUSTSEC-2021-0139, `deny` included. The one thing lost is the yanked-crate check, which shells out to `cargo -V` for the crates.io index protocol; the preset passes `--no-yanked` rather than print `couldn't update crates.io index` on every run — a yank is not an advisory, and a project wanting it back overrides `command`. No `git` binary is needed either way: the release binary fetches the advisory database itself. No exception moves — they are keyed by repository, and `rust:1.97.0-slim` still runs (#287)

- **presets**: give the `pip install` presets somewhere to write. cidx runs every container as the invoking uid, so pip cannot write the interpreter's site-packages and falls back to a user install under `$HOME` — which is `/` when `HOME` is unset, and the install dies with `Permission denied: '/.local'`. `mypy`, `bandit`, `pip-audit`, `pytest`, `python-build` and `twine` now carry `HOME = "/tmp"`, and the five that invoke a console script name `/tmp/.local/bin` on the way in, the way `govulncheck` names `/tmp/go/bin` — pip installs entry points there and no image puts it on `PATH`. Same family as #188, which gave `cargo-audit` `HOME`/`CARGO_HOME` for the same reason. The defect is as old as the presets and was invisible until #293: on the image they used to run, which ships no shell, all six died at container init before pip was ever reached (#238)

- **presets**: give `go-mod-tidy` a writable `GOPATH`. Its five sibling Go presets all pin `GOPATH = "/tmp/go"`; this one inherited the image default `/go`, which a non-root container cannot create in, so `go mod tidy` failed with `mkdir /go/pkg: permission denied` on any golang image — nothing to do with #293's repin, just never noticed (#238)

- **presets**: promote `dhi.io/trivy` 0.68 → 0.71, **51 HIGH / 2 CRITICAL → 0 / 0** (Trivy, DB of 2026-07-30). An ordinary promotion inside a live family, not a repin like #293's: dhi.io still publishes the unversioned trivy line. dhi.io dates no tag, so rule 2's 14-day cooldown is not measurable on it and rule 3's waiver is claimed out loud in its place — the image left behind carries two CRITICAL findings that affect us today. Grype, which counts 45 HIGH/CRITICAL on 0.68, still reports two on 0.71 (`GO-2026-5970` in `golang.org/x/text`, `GHSA-hrxh-6v49-42gf` in grpc, both advisory IDs Trivy does not carry); the audit will surface them as new findings. `cidx security vuln prune` removes nothing after the bump: no exception was ever filed against `dhi.io/trivy:0.68`, and all 33 entries stay carry-over with 21/21 catalogue images scanned (#238)

- **generate**: stop writing a stale, unpinned Go image into the GitLab workflows generated for users. `writeGitLabBootstrap` hardcoded `golang:1.23-alpine` — three Go releases behind the `go-version: "1.26"` the GitHub generator emits from the same `cidx.toml`, and a mutable tag on top of it. Both generators now read one `bootstrapGoVersion`, so the two cannot drift again, and the GitLab image is pinned by digest. Deliberately **not** the catalogue's Go image: `dhi.io/golang:...` is what this repo runs with the Docker Hardened Images credentials its own CI holds (#288), and anonymous clients get 401 — a workflow cidx generates is run by someone else, on their runners, and must not require a DHI entitlement to build the binary (guardrail 1). `golang:<version>-alpine` pulls for anyone (#238)

- **presets**: move the two Docker Hardened Images off the Alpine 3.21 line DHI abandoned. `dhi.io/golang:1.23-alpine3.21-dev` → `1.26.5-alpine-dev`, **340 HIGH / 23 CRITICAL → 0 / 0**, and the six Go presets stop running a toolchain three majors behind the `go 1.26` this repo compiles with — `govulncheck` could not even analyse the tree from the old image, which built it as go1.25 and then refused every package with `requires newer Go version go1.26`. `dhi.io/python:3.13-alpine3.21` → `3.13.14-alpine-dev`, **34 / 3 → 0 / 0**. Both land on the _unversioned_ alpine line rather than the `-alpine3.24…` family `frozen_variant` names: that report says what replaced the dead family, which is not the same as advice on what to pin, and `-alpine3.24-dev` freezes again the day DHI moves to Alpine 3.25 — the trap the catalogue just spent three Go releases in. A suffix carrying no version names the line rather than one base of it, so DHI republishes it against each new Alpine and `SupersedingVariant` can never call it frozen; the digest still pins the content, which is all rule 1 asks of the tag. Python also moves non-dev → `-dev`, because the line the catalogue was on ships no shell at all: every one of its six presets is a `sh -c 'pip install …'`, and every one of them died at container init with `exec: "sh": executable file not found`. dhi.io publishes no tag date, so rule 2's cooldown cannot be measured on it and this repin is the human act the policy reserves for that case; rule 3's waiver is claimed out loud in its place — the images left behind carry 374 HIGH / 26 CRITICAL findings affecting us today. DHI keeps one patch per minor, so there was no slightly older, equally clean tag to prefer as #277 and #280 could. The repin also settles 21 exceptions `cidx security vuln prune` had been holding as carry-over, because the two images were the last catalogue images carrying their CVEs: 17 on the Go image (the stdlib set CVE-2025-47907, -58187, -58188, -61723, -61725, and CVE-2025-58050) and 4 on the Python one (CVE-2025-9230). They are now obsolete and removed — `known-vulnerabilities.toml` goes from 54 entries to 33, all 33 still carry-over, none unknown (#238, #252)

- **ci**: check formatting instead of rewriting it. `[containers.prettier] write = true` made the `code` phase run prettier in write mode: it rewrote files and exited 0 whatever it found, so CI never failed on formatting drift and `cidx run code` mutated the working tree as a side effect of a check. The override is gone — the preset's `--check` default is what the checking phase runs — and writing moved to a `format` phase no pipeline lists, so it only ever runs when asked: `cidx run format`. Same split Go already had since #171, where golangci-lint's gofmt formatter gates CI and `go fmt` writes. A `.prettierignore` keeps generated files out of prettier's reach, starting with `SECURITY-BASELINE.md`, which the formatter and `cidx security baseline` had been rewriting in turn — 47 lines against the generator's 5, last writer winning (#272)

- **audit**: hand the runner's registry credentials to the scanners. Trivy and Grype run in containers that saw neither the docker config `docker/login-action` writes nor the daemon socket, so both re-pulled every image themselves as anonymous clients — which passes on public registries and answered `401 Unauthorized` on all five `dhi.io` images. A quarter of the catalogue produced no findings at all, and since `cidx security vuln prune` is fail-closed that pinned 122 of the 155 exceptions in `unknown`, where none of them could ever be shown obsolete. The config is now mounted read-only with `DOCKER_CONFIG` pointing at it, so credentials stay keyed by registry and only `dhi.io` receives them; and an image the scanner cannot authenticate to fails its leg instead of reporting a green skip (#284). With every catalogue image scanned, `cidx security vuln prune` can conclude for the first time: no exception is left in `unknown`, 101 obsolete entries are removed and the 54 whose CVE followed a promotion are kept for re-filing (#284)
